<pattern xmlns="http://purl.oclc.org/dsdl/schematron" abstract="true" id="condition">
  ...
  <rule context="$Invoice ">
    ...
    <assert test="$DT-CIUS-PT-165" flag="fatal" id="DT-CIUS-PT-165">[DT-CIUS-PT-165]-Invoice total amount with VAT (BT-112) = Invoice total amount without VAT (BT-109) + Invoice total VAT amount (BT-110), with an acceptance range of 1.00 € (it does not mean that this tolerance is accepted by the customer).</assert>
	<assert test="$DT-CIUS-PT-176_1" flag="fatal" id="DT-CIUS-PT-176">[DT-CIUS-PT-176]-An Invoice that contains Invoice lines (BG-25) with diferent VAT exemption reason codes, where the VAT category code (BT-151) is “Exempt from VAT”, shall contain exactly one VATBReakdown (BG-23) with the VAT category code (BT-118) equal to "Exempt from VAT", for each VAT exemption reason code (BT-121).</assert>
	<assert test="$DT-CIUS-PT-176_2" flag="fatal" id="DT-CIUS-PT-176">[DT-CIUS-PT-176]-An Invoice that contains Invoice lines (BG-25) with diferent VAT exemption reason codes, where the VAT category code (BT-151) is “Exempt from VAT”, shall contain exactly one VATBReakdown (BG-23) with the VAT category code (BT-118) equal to "Exempt from VAT", for each VAT exemption reason code (BT-121).</assert>
  </rule>
  <rule context="$Invoice_Line ">
	...
	<assert test="$DT-CIUS-PT-177" flag="fatal" id="DT-CIUS-PT-177">[DT-CIUS-PT-177]-The Item price base quantity (BT-149) shall be greater than zero.</assert>
  </rule>
  <rule context="$Payee ">
  ...
  <rule context="$Tax_Total ">
    <assert test="$DT-CIUS-PT-164" flag="fatal" id="DT-CIUS-PT-164">[DT-CIUS-PT-164]-Invoice total VAT amount (BT-110) = Σ VAT category tax amount (BT-117), with an acceptance range of 1.00 € (it does not mean that this tolerance is accepted by the customer).</assert>
	<assert test="$DT-CIUS-PT-175" flag="fatal" id="DT-CIUS-PT-175">[DT-CIUS-PT-175]-In a VATBReakdown (BG-23) where the VAT category code (BT-118) is "Exempt from VAT" the VAT category taxable amount (BT-116) shall equal the sum of Invoice line net amounts (BT-131) minus the sum of Document level allowance amounts (BT-92) plus the sum of Document level charge amounts (BT-99) where the VAT category codes (BT-151, BT-95, BT-102) are “Exempt from VAT", with an acceptance range of 1.00 € (it does not mean that this tolerance is accepted by the customer).</assert>
  </rule>
  <rule context="$Document_totals ">
    <assert test="$DT-CIUS-PT-160" flag="fatal" id="DT-CIUS-PT-160">[DT-CIUS-PT-160]-Sum of Invoice line net amount (BT-106) = Σ Invoice line net amount (BT-131), with an acceptance range of 1.00 € (it does not mean that this tolerance is accepted by the customer).</assert>
    <assert test="$DT-CIUS-PT-161" flag="fatal" id="DT-CIUS-PT-161">[DT-CIUS-PT-161]-Sum of allowances on document level (BT-107) = Σ Document level allowance amount (BT-92), with an acceptance range of 1.00 € (it does not mean that this tolerance is accepted by the customer).</assert>    
    <assert test="$DT-CIUS-PT-162" flag="fatal" id="DT-CIUS-PT-162">[DT-CIUS-PT-162]-Sum of charges on document level (BT-108) = Σ Document level charge amount (BT-99), with an acceptance range of 1.00 € (it does not mean that this tolerance is accepted by the customer).</assert>
    <assert test="$DT-CIUS-PT-163" flag="fatal" id="DT-CIUS-PT-163">[DT-CIUS-PT-163]-Invoice total amount without VAT (BT-109) = Σ Invoice line net amount (BT-131) - Sum of allowances on document level (BT-107) + Sum of charges on document level (BT-108), with an acceptance range of 1.00 € (it does not mean that this tolerance is accepted by the customer).</assert>
    <assert test="$DT-CIUS-PT-166" flag="fatal" id="DT-CIUS-PT-166">[DT-CIUS-PT-166]-Amount due for payment (BT-115) = Invoice total amount with VAT (BT-112) -Paid amount (BT-113) +Rounding amount (BT-114), with an acceptance range of 1.00 € (it does not mean that this tolerance is accepted by the customer).</assert>
  </rule>
  ...
  <rule context="$VAT_breakdown ">
    <assert test="$DT-CIUS-PT-167" flag="fatal" id="DT-CIUS-PT-167">[DT-CIUS-PT-167]-VAT category tax amount (BT-117) = VAT category taxable amount (BT-116) x (VAT category rate (BT-119) / 100), rounded to two decimals and with an acceptance range of 1.00 € (it does not mean that this tolerance is accepted by the customer).</assert>
  </rule>
  <rule context="$VATAA ">
  ...
  <rule context="$Invoice_Line_Price ">
    <assert test="$DT-CIUS-PT-170" flag="fatal" id="DT-CIUS-PT-170">[DT-CIUS-PT-170]-The BT-146 must equal the grossPrice (Item gross price) minus the discount (Item price discount), with an acceptance range of 1.00 € (it does not mean that this tolerance is accepted by the customer).</assert>
  </rule>
